...
public class BookingState extends AbstractState {
   ...
   /* public BookingState(ApplicationContext context) {
    ...
    private Set<Long> selectSeats(Event event, LocalDateTime airDate) {
        ...
        return inputSeats(freeSeats);
    }

    private Set<Long> inputSeats(List<Long> freeSeats) {
        Set<Long> set = null;
        do {
      set =  readInput("Input seats (comma separated): ", s -> 
            Arrays.stream(s.split(","))
                .map(String::trim)
                .mapToLong(Long::parseLong)
                .boxed().collect(Collectors.toSet()));
      Set<Long> alreadyBooked = set.stream().filter(e -> !freeSeats.contains(e)).collect(Collectors.toSet());
      if(alreadyBooked!=null &&  alreadyBooked.size()>0)
      {
    	  System.err.println("please select only from free seats ..");
    	  	set = null;
      }
        }while(set == null);
        return set;
    ...
    private <T extends DomainObject> T selectDomainObject(AbstractDomainObjectService<T> service, Function<T, String> displayFunction) {
        if (!service.getAll().isEmpty()) {
            ...
            long id = readLongInput("Input id (-1 for nothing): ");
